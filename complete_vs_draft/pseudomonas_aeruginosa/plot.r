...
conn_draft <- dbConnect(RSQLite::SQLite(), "ncomplete/Results/ncomplete.db")
...
plot(res_complete$start, res_complete$complexity, ylim = c(-max(res_complete$complexity), max(res_complete$complexity)),
     type="n", pch=3, cex = 0.3, col="gray50", lwd = 1, xlab="chromosome position, Mbp", 
     ylab="complexity", cex.lab=1.5, yaxt="n", axes=F)
axis(side=1, lwd = 0.5, col = "black", cex.axis=0.8, at = seq(1,6)*10^6, labels = seq(1,6), cex.axis = 1)
axis(side=2, at=c(0,5), lwd = 2, col = "gray20", cex.axis=0.8)
axis(side=4, at=c(0,-5), labels = c(0,5), lwd = 2, col = "blue", cex.axis=0.8)
